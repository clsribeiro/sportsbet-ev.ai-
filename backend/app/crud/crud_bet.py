...
async def get_bet_by_id(db: AsyncSession, *, bet_id: int, user_id: UUID) -> Bet | None:
    result = await db.execute(select(Bet).where(Bet.id == bet_id, Bet.user_id == user_id))
...
async def get_bets_by_user(db: AsyncSession, *, user_id: UUID, skip: int = 0, limit: int = 100) -> List[Bet]:
    result = await db.execute(
        select(Bet).where(Bet.user_id == user_id).order_by(Bet.placed_at.desc()).offset(skip).limit(limit)
    )
